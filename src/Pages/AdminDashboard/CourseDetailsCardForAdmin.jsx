...
const CourseDetailsCardForAdmin = ({ course, handleUpdateCourse, handleDeleteCousre }) => {
        ...
        <div>
            <div id={_id} className='border-2 border-blue-600 mb-5 rounded-xl grid grid-cols-1 md:grid-cols-7 lg:grid-cols-6 gap-5 overflow-hidden'>
                ...
                <div className='p-2 flex flex-col gap-3 justify-center items-center'>
                    <button onClick={() => handleUpdateCourse(course)} className='deafultButton'>Update Class</button>
                    <button onClick={() => handleDeleteCousre(course._id)} className='deafultButton'>Delete Course</button>
                </div>
            </div>
        </div>
    );
...
export default CourseDetailsCardForAdmin;